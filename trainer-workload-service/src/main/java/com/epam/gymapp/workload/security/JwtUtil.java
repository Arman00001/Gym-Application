...
public class JwtUtil {
    ...
    /**
     * Generates a JWT token to access services.
     *
     * <p>
     * The refresh token contains this service's name
     * </p>
     *
     * @return a signed JWT access token for services
     */
    public String generateServiceToken() {
        return JWT.create()
                .withSubject("gym-main-service")
                .withExpiresAt(new Date(System.currentTimeMillis() + JWT_ACCESS_TOKEN_VALIDITY))
                .withIssuedAt(Instant.now())
                .withClaim(ROLES, java.util.List.of("ROLE_SERVICE"))
                .sign(this.getAlgorithm());
    }
